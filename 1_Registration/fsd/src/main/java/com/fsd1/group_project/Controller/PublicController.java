...
// import com.fsd1.group_project.Service.UserIntegrationService;
    ...
    public ResponseEntity<?> signup(@RequestBody User user) {
        try {
            ...
            if (user.getPhone_no() == null || user.getPhone_no().length() < 10 || user.getPhone_no().length() > 10) {
                return ResponseEntity.badRequest().body(Map.of("error", "Phone no should be of 10 digits"
                        + user.getPhone_no() + "length is: " + user.getPhone_no().length()));
            }
            ...
            User savedUser = userService.saveNewUser(user); // Changed from saveNewUser to saveUser

            // Notify participation service about new user registration
            // userIntegrationService.notifyUserRegistration(savedUser);
        ...
        try {
            ...
            if (u.getResetToken() != null &&
                ...
                return ResponseEntity.badRequest()
                        .body(Map.of("error",
                                "A reset link has already been sent. Please wait before requesting another."));
            }
